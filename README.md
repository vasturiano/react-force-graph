...
| <b>nodePositionUpdate</b> | <i>func(nodeObject, coords, node)</i> | | Getter/setter for the custom function to call for updating the position of nodes at every render iteration. It receives the respective node `ThreeJS Object3d`, the coordinates of the node (`{x,y,z}` each), and the node's `data`. If the function returns a truthy value, the regular position update function will not run for that node. | | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: |
